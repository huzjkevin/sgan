...
def generator_step(args, batch, generator, discriminator, g_loss_fn, optimizer_g):
    ...
    scores_fake = discriminator(traj_fake, traj_fake_rel, cls_labels, seq_start_end) # TEST: cGAN
...
def check_accuracy(args, loader, generator, discriminator, d_loss_fn, limit=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_traj_fake_rel = generator(obs_traj, obs_traj_rel, cls_labels, seq_start_end)
            pred_traj_fake = relative_to_abs(pred_traj_fake_rel, obs_traj[-1])
